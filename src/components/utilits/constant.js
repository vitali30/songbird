const birdsData = [
  ...
  const URLs = {
    audioCorrect: 'https://birds-quiz.netlify.app/static/media/win.a1e9e8b6.mp3',
    audioError: 'https://birds-quiz.netlify.app/static/media/error.165166d5.mp3',
}
  const NEXT_LEVEL = 'Next Level';
  const REPEAT = 'Play Again';
  const MAX_STAGE = 6;
  const GRATITUTE = 'Поздравляем!';
  const END_GAME_TEXT1 = 'Вы прошли викторину набрав';
  const END_GAME_TEXT2 = 'из 30 возможных баллов';
  const NO_NAME = '******';
  const DEFAULT_IMAGE = 'https://birds-quiz.netlify.app/static/media/bird.06a46938.jpg';
  const QUESTION = 'question image';
  const DESCRIPTION = 'description image';
  const LISTEN = 'Послушайте плеер';
  const CHOOSE = 'Выберите птицу из списка'
